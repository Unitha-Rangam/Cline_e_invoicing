...
define(['N/runtime', 'N/https', 'N/log'], (runtime, https, log) => {
    ...
    log.debug('Scriptlog', 'PARTNER_IDENTIFIER_PARAM' +PARTNER_IDENTIFIER_PARAM);
    ...
    async function getInputData() {
        try {
            ...
            if (response.code !== 200) throw new Error('Failed to call Outbound List RESTlet');
            ...
            return result.data.items.map(r => r.messageId);
        } catch (err) {
    ...
    async function map(context) {
        try {
            ...
            const body = { messageId, partnerIdentifier: partnerId };
